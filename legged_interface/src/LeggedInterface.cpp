...
#include "legged_interface/constraint/ZeroForceConstraint_tail.h"
...
void LeggedInterface::setupOptimalControlProblem(const std::string& taskFile, const std::string& urdfFile, const std::string& referenceFile,
  ...
  RelaxedBarrierPenalty::Config TailPenalty;
  RelaxedBarrierPenalty::Config FootPenalty;
  SmoothAbsolutePenalty::Config ArmPenalty;

  boost::property_tree::ptree pt_tail;
  boost::property_tree::read_info(taskFile, pt_tail);
  const std::string prefix_tail = "TailPositionLimit.";
  loadData::loadPtreeValue(pt_tail, TailPenalty.mu, prefix_tail + "mu", verbose);
  loadData::loadPtreeValue(pt_tail, TailPenalty.delta, prefix_tail + "delta", verbose);

  boost::property_tree::ptree pt_foot;
  boost::property_tree::read_info(taskFile, pt_foot);
  const std::string prefix_foot = "FootPositionLimit.";
  loadData::loadPtreeValue(pt_foot, FootPenalty.mu, prefix_foot + "mu", verbose);
  loadData::loadPtreeValue(pt_foot, FootPenalty.delta, prefix_foot + "delta", verbose);

  boost::property_tree::ptree pt_arm;
  boost::property_tree::read_info(taskFile, pt_foot);
  const std::string prefix_arm = "ArmPositionLimit.";
  loadData::loadPtreeValue(pt_arm, ArmPenalty.scale, prefix_arm + "mu", verbose);
  loadData::loadPtreeValue(pt_arm, ArmPenalty.relaxation, prefix_arm + "delta", verbose);

  const std::string& footName = modelSettings_.contactNames3DoF[4];
 
  std::unique_ptr<EndEffectorKinematics<scalar_t>> tailKinematicsPtr = getEeKinematicsPtr({footName}, footName);
  problemPtr_->equalityConstraintPtr->add(footName + "_zeroForce", std::unique_ptr<StateInputConstraint>(new ZeroForceConstraint_tail(
                                                                         *referenceManagerPtr_, 4, centroidalModelInfo_)));
  // problemPtr_->softConstraintPtr->add(footName + "tail_pos", getTailPositionSoftConstraint(*tailKinematicsPtr, 4, TailPenalty));

  // problemPtr_->softConstraintPtr->add(footName + "arm_pos", getArmPositionSoftConstraint(*tailKinematicsPtr, 4, ArmPenalty));
  ...
  for (size_t i = 0; i < centroidalModelInfo_.numThreeDofContacts-1; i++) {
    ...
    // problemPtr_->softConstraintPtr->add(footName + "foot_pos", getFootPositionSoftConstraint(*eeKinematicsPtr, i, FootPenalty));
...
std::unique_ptr<StateInputCost> LeggedInterface::getTailPositionSoftConstraint(const EndEffectorKinematics<scalar_t>& eeKinematics, size_t contactPointIndex,
                                                                               const RelaxedBarrierPenalty::Config& barrierPenaltyConfig) {
  return std::make_unique<StateInputSoftConstraint>(std::make_unique<TailPositionConstraint>(*referenceManagerPtr_, eeKinematics, contactPointIndex),
                                                    std::make_unique<RelaxedBarrierPenalty>(barrierPenaltyConfig));
}

std::unique_ptr<StateInputCost> LeggedInterface::getArmPositionSoftConstraint(const EndEffectorKinematics<scalar_t>& eeKinematics, size_t contactPointIndex,
                                                                               const SmoothAbsolutePenalty::Config& QuadraticPenaltyConfig) {
  return std::make_unique<StateInputSoftConstraint>(std::make_unique<ArmPositionConstraint>(*referenceManagerPtr_, eeKinematics, contactPointIndex),
                                                    std::make_unique<SmoothAbsolutePenalty>(QuadraticPenaltyConfig));
}

std::unique_ptr<StateInputCost> LeggedInterface::getFootPositionSoftConstraint(const EndEffectorKinematics<scalar_t>& eeKinematics, size_t contactPointIndex,
                                                                               const RelaxedBarrierPenalty::Config& barrierPenaltyConfig) {
  return std::make_unique<StateInputSoftConstraint>(std::make_unique<FootPositionConstraint>(*referenceManagerPtr_, eeKinematics, contactPointIndex),
                                                    std::make_unique<RelaxedBarrierPenalty>(barrierPenaltyConfig));
}
/******************************************************************************************************/
/******************************************************************************************************/
